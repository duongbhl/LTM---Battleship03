...
static int try_match_elo_locked(void)
{
    ...
    for (int i = 0; i < q_elo_len; i++)
    {
        ...
        if (wi >= 30)
            continue; // sẽ sweep sang open ở bước khác
...
void mm_request_match(int sock, const char *user, int elo, int elo_mode)
{
    ...
    // check đã trong queue chưa (an toàn)
    ...
    if (!u || u->state != STATE_IDLE)
    {
        pthread_mutex_unlock(&mm_lock);
        send_logged(sock, "ERROR|Cannot queue now\n");
        return;
    }

    if (gs_find_by_player(sock))
    {
    ...
    if (elo_mode == 1)
    {
        ...
        {
            pthread_mutex_unlock(&mm_lock);
            send_logged(sock, "ERROR|Queue full\n");
            return;
        }
    }
    else
    {
        // OPEN MODE: match ngay hoặc enqueue
        u->state = STATE_QUEUE;
        match_open_rank_locked(me); // hàm này PHẢI giả định mutex đang lock
    }
